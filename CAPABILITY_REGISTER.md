...
- `implemented_v0.2.2`: documented inventory/planning slice for v0.2.2 (no runtime code).
...
| factor-core implementation | `planned_v0.3` | Scan kernel, export script, golden parity; not started in v0.2.2. |
| constitution + ADR + CI | `implemented_v1` | Constitution, ADR-0001..0010 (0010 proposed), ruff, pytest. |
...
| factor tracking | `not_in_v0.2.x` | Sample-out tracking; separate from factor-core; future ADR. |
...
- v0.2.2 delivered: factor-core **inventory + golden scenario planning** (docs only).
- v0.3 candidate: factor-core **implementation** (scan + export + golden parity).
