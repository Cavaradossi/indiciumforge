# IndiciumForge Capability Register
...
| artifact manifest / audit CLI | `implemented_v0.4.1` | Scan + validate market_gate and daily_review stage dirs; `indiciumforge artifact list/audit`; ADR-0008, ADR-0014. |
...
| session-aware data provider v2 | `implemented_v0.9` | `DataProviderPortV2`, `ProviderRegistryV2`, pack loader, `indiciumforge provider inspect/fetch`; ADR-0019/0020. |
...
| private factor pack loading integration | `implemented_v0.7` | `load_factor_pack`, pack YAML, entry points; `indiciumforge factor scan`; ADR-0017. |
...
| A-share private recipe integration | `implemented_v0.10` | `indiciumforge_core.recipes`, RecipeRunner, extension pack loader, recipe-driven chain CLI; ADR-0021. |
| private local parity harness | `implemented_v0.11` | `indiciumforge_core.parity`, `indiciumforge parity run/report`, synthetic demo reference; ADR-0022. |
| A-share private extension path (IG-output adapter) | `signed_v1.0` | External `indiciumforge-private-ashare`; v1.0 sign-off golden date evidence; production review builder remains `private_extension`. |
| proprietary long-structure detectors | `private_extension` | Real IG detector rules; private factor packs only. |
| market daily-review upstream | `implemented_v0.4.1` | Skeleton `theme_state_ranking` generation + `indiciumforge workflow daily-review` CLI; ADR-0013/0014; synthetic fixtures only. |
| synthetic end-to-end workflow | `implemented_v0.5_alpha` | `indiciumforge workflow synthetic-e2e`; DR -> MG -> audit summary; ADR-0015; fixture-only demo. |
| post-close / preopen workflow chain | `implemented_v0.6` | Skeleton chain: DR -> post_close -> preopen -> MG; `indiciumforge workflow chain`; ADR-0016; synthetic fixtures. |
...
- v0.4.1 delivered: **daily-review CLI** (`indiciumforge workflow daily-review`) and **manifest audit**
  for `market_awareness/{YYYYMMDD}/daily_review/` (ADR-0014).

## v0.5-alpha vertical slice (completed)

- v0.5.0 delivered: **synthetic end-to-end workflow** (`indiciumforge workflow synthetic-e2e`);
  daily-review skeleton -> market-gate -> dual-stage audit -> summary JSON (ADR-0015).

## v0.6 vertical slice (completed)

- v0.6.0 delivered: **workflow chain skeleton** (`indiciumforge workflow chain`);
  daily-review -> post_close -> preopen -> market-gate -> audit -> summary JSON (ADR-0016).

## v0.7 vertical slice (completed)

- v0.7.0 delivered: **private factor pack loading integration** (`indiciumforge factor scan`) and
...
- v0.8.0 delivered: **session-cyclic workflow model** — recipe/checkpoint/handoff contracts,
  `indiciumforge.workflow_recipe.v1`, A-share recipe fixture, summary v3 session metadata (ADR-0018).

## v0.9 vertical slice (completed)

- v0.9.0 delivered: **session-aware data provider contract v2** — `DataProviderPortV2`,
  `ProviderRegistryV2`, provider pack loading, `indiciumforge provider inspect/fetch`, fake private
...
- v0.10.0 delivered: **A-share private recipe integration** — `indiciumforge_core.recipes` ports,
...
- v0.11.0 delivered: **private local parity harness** — `indiciumforge_core.parity`,
  `ReferenceArtifactProvider`, `CandidateComparator`, `indiciumforge parity run/report`,
...
- Accepted gaps in private `V1_0_SIGNOFF_GAP_REGISTER.md` (no IndiciumForge Git paths).
