...
- `planned_v0.2`: ADR proposed or accepted; implementation targeted for v0.2 vertical slice.
...
| constitution + ADR + CI | `implemented_v1` | Constitution, ADR-0001..0008 (0008 proposed), ruff, pytest. |
| artifact manifest / audit CLI | `planned_v0.2` | Read-only scan + validate market-gate stage dirs; ADR-0008. |
| data provider port | `contract_only` (`planned_v0.2.1`) | v0.1: contract tests with fixture registry only. v0.2.1: Protocol v1 + LocalFixtureProvider; no workflow wiring. |
...
- v0.2 slice: **C only** (artifact manifest / audit CLI).
