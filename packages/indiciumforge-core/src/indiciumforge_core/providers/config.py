...
def load_providers_from_entry_points(
    group: str = "indiciumforge.data_providers",
...
def _parse_pack_config(path: Path) -> dict[str, Any]:
    ...
    if not accepts_schema(schema, PROVIDER_PACK_SCHEMA, context=str(path)):
...
def load_provider_pack(
    *,
    ...
    entry_point_group: str = "indiciumforge.data_providers",
) -> LoadedProviderPack:
