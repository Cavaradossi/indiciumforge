...
\begin{enumerate}[leftmargin=*]
  ...
  \item Real quant capability (W4): four ports--analytics, portfolio, backtest, pricing--with reference adapters, an A-share golden snapshot, and a deterministic end-to-end pipeline behind \texttt{indiciumforge quant}.\claim{C43-C51, ADR-0026}
\end{enumerate}
...
\paragraph{Quant analytics, optimization, backtest and pricing (W4).}
...
W4 quant pipeline & Deterministic factor$\to$analytics$\to$optimize$\to$backtest on the synthetic golden panel & C43-C51, ADR-0026 \\
...
\paragraph{W4 reference quant pipeline on the synthetic golden panel.}
W4 instantiates the four quant ports with reference adapters and wires them into an
...
\bottomrule
\end{tabularx}
\caption{W4 factor analytics on the synthetic golden panel (n = 501 dates per horizon).
...
The W4 \texttt{quant} group runs without the optional \texttt{[analytics]}/\texttt{[portfolio]}
...
The W4 quant engine is scoped deliberately: the vectorized backtester is daily,
...
and qlib integration are out of scope for W4; the ports remain swappable for alternative
adapters.
