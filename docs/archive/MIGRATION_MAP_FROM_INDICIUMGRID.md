...
| --- | --- | --- | --- |
| `workflow.apply_market_gate` | `indiciumforge_workflow.market_gate.kernel` | implement | `strict_pass_mixed` |
| `workflow._market_gate_row` | market-gate row evaluation | implement | `strict_pass_mixed` |
| active-watch helpers | `indiciumforge_workflow.market_gate.active_watch` | implement | `empty_strict_c_grade` |
| calibration audit helper | `indiciumforge_workflow.market_gate.calibration` | implement | all market-gate scenarios |
| market-gate review path resolver | `indiciumforge_workflow.market_gate.resolver` | implement | `fallback_post_close` |
| `workflow.run_market_gate_workflow` | `indiciumforge_workflow.market_gate.runner` | implement | all market-gate scenarios |
| `REVIEW_COLUMNS` / `MARKET_GATE_COLUMNS` | `indiciumforge_core.labels` | implement compat labels | all market-gate scenarios |
| `THEME_STATE_RULES` | `indiciumforge_core.market.theme_rules` | metadata only | state/summary artifacts |
| provider registry | `indiciumforge_core.providers.registry` + `DataProviderPort` | implement v0.2.1 | contract tests |
| IG `data/registry.py` priority model | `indiciumforge_core.providers.registry_v2` + `DataProviderPortV2` | implement v0.9 | authority + fallback tests |
| LocalFixtureProvider | `indiciumforge_core.providers.local_fixture` | implement v0.2.1 | `tests/fixtures/ohlcv` |
| LocalFixtureProvider v2 | `indiciumforge_core.providers.local_fixture_v2` | implement v0.9 | session query + pack demo |
...
| `run_factor_scan` | `indiciumforge_core.factors.scan.FactorScanRunner` | implemented v0.7 integration | FACTOR_DEMO_MANIFEST |
...
| `evaluate_factor_parameters` | `indiciumforge_core.factors.evaluation` (planned) | inventory v0.2.2; calibrated params private-extension | defer v0.3+ |
...
| `market_awareness` daily-review | `indiciumforge_workflow.market_awareness` + `indiciumforge workflow daily-review` | implemented v0.4.1 | `theme_state_ranking` + manifest audit |
| synthetic E2E workflow | `indiciumforge_workflow.e2e.synthetic` + `indiciumforge workflow synthetic-e2e` | implemented v0.5-alpha | DR -> MG -> audit summary demo |
| workflow chain skeleton | `indiciumforge_workflow.workflow_chain` + `indiciumforge workflow chain` | implemented v0.6 | DR -> post_close -> preopen -> MG |
| IG workflow stage folders | `indiciumforge_core.workflow` recipe stage ids | implemented v0.8 | `recipe_ashare_daily_v1.yaml`; ADR-0018 |
...
| IG `output/workflows/` golden slices | `indiciumforge_core.parity` + local `reference_artifact_root` | implemented v0.11 | private-local harness; ADR-0022 |
...
IndiciumForge slices. Review them before migrating factor-core, factor-tracking, provider, account, or
...
Migration rule: ignored paths may provide evidence, schemas, or scenario seeds, but IndiciumForge should
