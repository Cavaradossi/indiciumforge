...
- Market-gate consumes `market_awareness/{YYYYMMDD}/daily_review/theme_state_ranking.csv` as a hard
  prerequisite ([runner.py](../../packages/indiciumforge-workflow/src/indiciumforge_workflow/market_gate/runner.py)).
- IndiciumForge v0.2 through v0.3 built artifact audit, provider port, and factor detector port foundations.
- MIGRATION_ROADMAP v0.4-a targets upstream generation of the gate-minimal theme ranking artifact.

## Decision

IndiciumForge v0.4-a implements a **skeleton** daily-review upstream in `indiciumforge_workflow.market_awareness`:
...
Uses existing [theme_rules.py](../../packages/indiciumforge-core/src/indiciumforge_core/market/theme_rules.py)
...
State JSON schema: `indiciumforge.market_daily_review_state.v1` (IndiciumForge-native, not IG schema).
...
- CLI `indiciumforge workflow daily-review` (v0.4-b)
