...
除研究工作流外，IndiciumForge 现在内置**真实可测试的量化能力**（Route A，`indiciumforge_core.quant`），沿用同一套端口 + 包加载模式：

| 能力 | 端口 / 适配器 | 说明 |
| --- | --- | --- |
| 因子分析 | `FactorAnalyticsPort` / `StatsmodelsFactorEngine` | 分周期 Spearman IC、Fama-MacBeth 斜率与 t 值、换手率 |
| 组合优化 | `PortfolioOptimizationPort` / `CvxpyOptimizer` | 均值-方差 / 最小方差，多空约束、个股权重上限、行业上限 |
| 回测 | `BacktestPort` / `VectorizedBacktester` | 纯 numpy/pandas 向量化；使用**前一周期权重**（无前视）、固定成本、Sharpe/最大回撤/Calmar |
| 期权定价 | `PricingPort` / `BlackScholesPricer` | 解析 Black-Scholes 欧式价格与希腊字母（仅标准库，无额外依赖） |
| 数据 | `GoldenSnapshotProvider` / `AkshareDataProvider` | 提交式合成 A 股黄金面板；akshare 适配器置于 `data` extra 与离线 `cache_only` 模式 |

CLI 通过 `indiciumforge quant` 暴露 `analytics` / `optimize` / `backtest` / `price` / `pipeline` 五个子命令；重依赖在命令体内部惰性导入，因此**未安装 `analytics`/`portfolio` extra 也能加载 CLI**。

```bash
# 端到端流水线：因子 → 分析 → 优化 → 回测（确定性，运行于合成黄金面板）
indiciumforge quant pipeline \
  --panel tests/fixtures/golden_ashare/panel.parquet \
  --rebalance-every 10

# 解析 Black-Scholes 欧式期权定价 + 希腊字母
indiciumforge quant price \
  --spot 100 --strike 100 --maturity 1 --rate 0.05 --volatility 0.2 --type call
```

> `analytics` / `optimize` / `backtest` 子命令接受**预构建的 CSV**（因子面板、收益面板、期望收益、协方差、权重历史等）；`pipeline` 直接从 OHLCV 面板出发，是开箱即用的端到端演示。

> **诚实声明**：W4 回测为**日频、单资产收益、固定成本**模型，不含滑点、市场冲击或日内动态；演示使用**合成**黄金面板，所报指标仅用于验证框架正确性与流水线接线，**不构成**市场表现或实盘回测，也**不构成**投资建议（见 ADR-0026）。 QuantLib / rqalpha 等外部框架集成不在 W4 范围，端口保持可替换。
...
- W4 量化流水线（`indiciumforge quant`）运行在**合成黄金面板**上，仅用于演示框架能力，不构成市场表现或投资建议；回测为日频、固定成本模型，不含滑点/冲击（见 ADR-0026）；
