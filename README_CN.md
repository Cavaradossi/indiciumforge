...
除研究工作流外，IndiciumForge 现在内置**真实可测试的量化能力**（`indiciumforge_core.quant`），沿用同一套端口 + 包加载模式：
...
> **诚实声明**：回测为**日频、单资产收益、固定成本**模型，不含滑点、市场冲击或日内动态；演示使用**合成**黄金面板，所报指标仅用于验证框架正确性与流水线接线，**不构成**市场表现或实盘回测，也**不构成**投资建议（见 ADR-0026）。 QuantLib / rqalpha 等外部框架集成不在本版本范围，端口保持可替换。
...
- 量化流水线（`indiciumforge quant`）运行在**合成黄金面板**上，仅用于演示框架能力，不构成市场表现或投资建议；回测为日频、固定成本模型，不含滑点/冲击（见 ADR-0026）；
